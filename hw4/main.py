...
PATH_ATTRS = ['observations', 'actions', 'rewards', 'next_observations']
...
def sample(env,
    ...
    for i in range(num_paths):
        ...
        for t in range(horizon):
            ...
            if log and t % 10 == 0:
                logger.info('Sampling timestep %i', t)
            ...
            if done:
                # TODO(jalex): pad these paths to make sure same length
                break
...
def flatten_paths(paths):
    ...
    data = {}
    for element in PATH_ATTRS:
        data[element] = np.vstack([path[element] for path in paths])
...
def shuffle(data):
    indices = np.random.permutation(data['observations'].shape[0])

    data_shuffled = {k: v[indices] for k, v in data.items()}
...
def train(env,
    ...
    if dpath_resume:
        idx_start, data = logz.continue_training(dpath_resume)
    else:
        idx_start = 0
    ...
    paths = sample(env, random_controller, num_paths_random, env_horizon, render, log=False)
    data = flatten_paths(paths)
    ...
    for itr in range(onpol_iters):
        ...
        paths_onpol = sample(env, mpc_controller, num_paths_onpol, env_horizon, render)

        costs = [path_cost(cost_fn, path) for path in paths_onpol]
        returns = [path['rewards'].sum() for path in paths_onpol]

        data_onpol = flatten_paths(paths_onpol)

        for x in data:
            data[x] = np.vstack((data_onpol[x]))
        ...
        logz.save_tf_model(sess, step=itr)
        logz.pickle_data(data, step=itr)
